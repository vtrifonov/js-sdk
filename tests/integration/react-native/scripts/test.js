  ...
  return build(files)
    .then(function (results) {
      return Promise.all(results
        .map(function (result) {
          result.code = result.code.replace('__SDK__', 'kinvey-react-native-sdk');
          result.code = `global.before = global.beforeAll;\nglobal.after = global.afterAll;\n\n${result.code}`;
          return result;
        })
        .map(function (result) {
...
fs.remove(TEST_DIR)
  .then(() => {
    return []
      ...
      .concat(glob.sync(path.join(SPECS_DIR, 'react-native/**/*.js')));
  })
